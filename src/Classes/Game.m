...
- (void)setup
{
    ...
    SPTexture *buttonTexture = [SPTexture textureWithContentsOfFile:@"atlas.png"];
    SPButton *_Button = [[SPButton alloc] initWithUpState:buttonTexture text:@"TakePhoto"];
    _Button.x = Sparrow.stage.width *0.5;
    _Button.y = Sparrow.stage.height *0.5;
    _Button.name = @"credit";
    [_Button addEventListener:@selector(onButtonTrigger:) atObject:self
                            forType:SP_EVENT_TYPE_TRIGGERED];
    [self addChild:_Button];
}
- (void)onButtonTrigger:(SPEvent *)event
{
    ViewController *_viewController = (ViewController*)Sparrow.currentController;
    _viewController._game = self;
    [_viewController takePhoto];
}
-(void) setImage:(UIImage *)image
{
    
    SPTexture* mTexture = [[SPTexture alloc] initWithContentsOfImage:image];
    SPImage* mImage = [[SPImage alloc] initWithTexture:mTexture];
